...
export default function RegisterScreen() {
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      actions.register(token, user);
      navigate('/verify');
    } catch (err) {
      setApiError(err.message || 'Registrierung fehlgeschlagen.');
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <Screen background="var(--surface)">
      ...
      <div style={{ padding: '12px 22px 24px', borderTop: '1px solid var(--line)', background: 'var(--card)', display: 'flex', flexDirection: 'column', gap: 8, flexShrink: 0 }}>
        {apiError && (
          <div style={{ fontFamily: 'var(--font)', fontSize: 13, color: 'var(--danger)', textAlign: 'center' }}>{apiError}</div>
        )}
